...
# Legacy fragments that mean an existing root file was generated by an older
# template. Such a file is refreshed from the canonical source even without
# --force, so the filesystem and the saved project never disagree.
...
# Fragments the *current* templates guarantee. If a root file lost any of them
# it was produced by an older template and is refreshed even without --force.
...
def _is_stale(destination: str, existing: str) -> bool:
    if any(marker in existing for marker in STALE_MARKERS.get(destination, ())):
        return True
    return any(
        marker not in existing
        for marker in REQUIRED_MARKERS.get(destination, ())
    )
...
def _write(path: Path, content: str, force: bool) -> str:
    try:
        ...
        if path.exists() and not force:
            ...
            elif _is_stale(path.name, existing):
                # Older generated form (e.g. CMD ["./start.sh"]): refresh it.
                path.write_text(content)
                result = f"wrote (was outdated): {path}"
            else:
